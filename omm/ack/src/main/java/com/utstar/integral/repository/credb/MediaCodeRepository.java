...
public class MediaCodeRepository {
    ...
    public Map<String, Set<String>> getMediaCodeByBasetag(List<String> basetagCodes, List<String> sysids) {
        ...
        nativeQuery.setParameter("basetagCodes", basetagCodes);
        List<Object> resultList = nativeQuery.getResultList();
        if (CollectionUtils.isEmpty(resultList)) {
    ...
    public Map<String, Set<String>> getMediaCodeByChargetypes(List<String> chargetypes, List<String> sysids) {
        Map<String, Set<String>> result = new HashMap<>();
        List<String> collect = chargetypes.stream().
                filter(item -> !item.equals("2")).collect(Collectors.toList());
        for (String sysid : sysids) {
            Set<String> codeSet = getMediaCodeByChargetype(collect, sysid);
            if (codeSet.isEmpty()) continue;
            result.put(sysid, codeSet);
        }
    ...
    public List<VueTreeEntity> getCategoryByPid(Integer pid) {
        String sql = "SELECT CATEGORYID,PARENTID,NAME FROM BTO_C2.CATEGORY WHERE SEQ IS NOT NULL START WITH PARENTID IS NULL CONNECT BY PRIOR CATEGORYID = PARENTID ORDER BY SEQ";
        /*String sql = null;
        if(pid == null)
        {
            sql = "SELECT CATEGORYID,NAME FROM BTO_C2.CATEGORY WHERE LEVEL = 1 START WITH PARENTID IS NULL CONNECT BY PRIOR CATEGORYID = PARENTID";
        }else
        {
        ...
        List<VueTreeEntity> result = handle(resultList);
        return result;
    }

    private List<VueTreeEntity> handle(List<VueTreeEntity> resultList) {

        Map<Integer, List<VueTreeEntity>> map = new HashMap<>();
        resultList.forEach(item -> {
            if (item.getParentId() == null) {
                List<VueTreeEntity> list = map.get(-1);
                if (list == null) {
                    list = new ArrayList<>();
                    map.put(-1, list);
                }
                ...
                List<VueTreeEntity> list = map.get(pid);
                if (list == null) {
                    list = new ArrayList<>();
                    map.put(pid, list);
                }
        ...
        List<VueTreeEntity> result = map.get(-1);
        result.forEach(item -> {
            List<VueTreeEntity> childrens = findChildren(item.getId(), map);
            item.setChildren(childrens);
        });
        List<VueTreeEntity> collect = result.stream().filter(item ->
                !CollectionUtils.isEmpty(item.getChildren())
        ).collect(Collectors.toList());
        return collect;

    }

    private List<VueTreeEntity> findChildren(Integer id, Map<Integer, List<VueTreeEntity>> map) {
        List<VueTreeEntity> vueTreeEntities = map.get(id);
        if (vueTreeEntities != null) {
            for (VueTreeEntity vue : vueTreeEntities) {
                List<VueTreeEntity> chidrens = findChildren(vue.getId(), map);
                vue.setChildren(chidrens);
            }
        }
        return vueTreeEntities;
    }
    ...
    private String getChargetypeSql(List<String> chargetypes, String sysid) {
        ...
        } else if ("1".equals(chargetypes.get(0)))//点播收费
        {
            sqlType += " AND " + sysid + "chargetype = '0'";
        } else if ("0".equals(chargetypes.get(0)))//点播免费
        {
            sqlType += " AND " + sysid + "chargetype != '0'";
        } else {
            return null;
