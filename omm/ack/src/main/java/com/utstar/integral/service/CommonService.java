            ...
            if(s.length == 8){
                ...
                if(StringUtils.isNotBlank(activityId)){
                    ...
                    if(activityEntity != null && activityEntity.getStatusType().equals(ActivityStatusType.VALID)
                            && (activityEntity.getValidTime().getTime() < System.currentTimeMillis()
                            && activityEntity.getExpireTime().getTime() > System.currentTimeMillis())
                            && (!activityEntity.getDataType().equals(ActivityDataType.TYPE) || !activityEntity.getMediaCode().equals("p"))){
                        if(map.containsKey(activityId)) continue;
    ...
    private void handleLog(List<String> list){
        ...
        list.forEach(log ->{
            if(StringUtils.isNotBlank(log)){
                ...
                if(logArr != null && logArr.length == 4){
                    LoggerBuilder.getLogger(logArr[0]).info("{},{},{}",logArr[1],logArr[2],logArr[3]);
                }
            }
        });
        list.clear();
