...
$.fn.SetTextControls = function (data) {
    var $id = $(this);
    for (var key in data) {
        var id = $id.find("#" + key);
        if (id.attr("id")) {
            ...
            switch (type) {
                case "date":
                    id.html(formatDate(value, "yyyy-MM-dd"));
                    break;
                case "text":
                    id.html(value);
                    break;
...
//        if(id== defined){
        if (id.attr("id")) {
            ...
            if (!type && id[0] && (id.hasClass("select2"))) {
                type = "select2";
            }
            if (type && id[0] && (id.hasClass("check-switch"))) {
...
$.fn.addTabsForChildern = function (options) {
    var defaults = {
        id: null,
        title: '系统窗口',
        url: '',
...
$.fn.modalOffset = function (height, isTop) {
    var htmlHeight;
    if (isTop === false) {
        htmlHeight = $(document).find('html')[0].clientHeight;
    } else {
        htmlHeight = $(top.document).find('html')[0].clientHeight;
    }
    var offset = (parseInt(htmlHeight) - height) / 2;
    return offset + 'px';
};

/**
 * 打开弹出框
 * @param options 弹出框的参数
 * @param isTop 弹出框基于根部窗口
 */
$.fn.modalOpen = function (options, isTop) {
    var defaults = {
        id: null,
        title: '系统窗口',
        width: "100px",
    ...
    if (options.offset === undefined) {
        options.offset = $.fn.modalOffset(parseInt(options.height.replace('px', '')), isTop);
    }
    ...
    openCheck({
        code: function () {
            var layerOption = {
                ...
                isOutAnim: options.isOutAnim,
                offset: options.offset,
                yes: function (index, layero) {
                    if (!options.callBack) {
                        return;
                    }
                    var flag = options.callBack("layui-layer-iframe" + index, layero);
                    if (flag === true) {
                        layero.find('.layui-layer-btn0').unbind("click");
                        if (isTop === false) {
                            layer.close(index);
                        } else {
                ...
                btn2: function (index, layero) {
                    ...
                    var flag = options.callBack2("layui-layer-iframe" + index, layero);
                    if (flag === true) {
                        layero.find('.layui-layer-btn1').unbind("click");
                        return true;
                    } else {
                        return false;
                    }
                ...
                btn3: function (index, layero) {
                    if (!options.callBack3) {
                        return true;
                    }
                    var flag = options.callBack3("layui-layer-iframe" + index, layero);
                    if (flag === true) {
                        layero.find('.layui-layer-btn2').unbind("click");
                        return true;
                    } else {
                        return false;
                    }
                ...
                btn4: function (index, layero) {
                    if (options.callBack4) {
                        return true;
                    }
                    var flag = options.callBack4("layui-layer-iframe" + index, layero);
                    if (flag === true) {
                        layero.find('.layui-layer-btn3').unbind("click");
                        return true;
                    } else {
                        return false;
                    }
...
$.fn.modalMsg = function (content, type, offset) {
    var iconType = 0;
    if (type != undefined) {
        var icon = "";
        if (type == 'success') {
        ...
        var options = {icon: iconType, time: 2000, shift: 5, shade: 0};
        if (offset != undefined) {
            options.offset = offset;
        }
        top.layer.msg(content, options);
...
/**
 * success方法返回值true就不通过以下提示；false与undefined就通过以下提示。
 * @param data
 */
$.fn.commonMessage = function (data) {
    ...
    if (data.code == 9999) {
        $.fn.modalMsg("无权访问", "danger");
        return;
    }
    if (data.message != undefined || data.message != null) {
        $.fn.modalMsg(data.message, "danger");
        return;
    }
    
    /*if (typeof errCodeDef != undefined && typeof errCodeDef[data.code] != undefined){
    	$.fn.modalMsg(errCodeDef[data.code], "danger");
        return;
    }*/
    ...
    if (code >= 10400 && code <= 10499) {
        data.message = '内容包错误码';
        if(code == 10400){
        	data.message = '系统发生未知异常';
        }
    }
    if (code >= 10500 && code <= 10599) {
        data.message = '图文节目';
    }
    if (code >= 10600 && code <= 10699) {
        data.message = '外部链接';
    }
    if (code >= 20000 && code <= 20099) {
        data.message = '编排错误码';
    }
    if (code >= 20100 && code <= 20199) {
        data.message = '展示错误码';
    }
    if (code >= 20200 && code <= 20299) {
        data.message = '静态化错误码';
    }
    if (typeof data.message != "undefined" && data.message != "undefined" && data.message != "") {
        $.fn.modalMsg(data.message, "danger");
    }
}
...
$.fn.submitForm = function (options) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            var flag = options.success(data);
            if (flag == undefined || !flag) {
                $.fn.commonMessage(data);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
...
$.fn.deleteForm = function (options) {
    ...
    var options = $.extend(defaults, options);
    $.fn.modalConfirm(options.prompt, function (r, index, callBack) {
        if (r) {
            $.loading(true, options.loading);
            $.ajax({
                url: options.url,
                ...
                success: function (data) {
                    ...
                    callBack.find('.layui-layer-btn0').unbind("click");
...
$.fn.turnOffForm = function (options) {
    var defaults = {
        prompt: "注：您确定要对该用户进行停机或复机操作吗？",
        url: "",
        param: [],
        loading: "正在对该用户进行操作...",
        success: null,
...
/**
 * 每个执行top.layer.open方法时需要先检查session是否过期
 * @param options
 */
openCheck = function (options, path) {
    console.log(projectName);
    $.ajax({
        type: 'post',
        url: projectName + "/admin/sessionInfo/checkSession.do",
        async: false,
        dataType: 'json',
        success: function (data) {
            if (data.code == "1") {
                ...
                p.location.href = projectName;
            } else if (data.code == "0") {
                openPrivilege(options, path);
            }
        }
    });
 ...
 * @param path
 */
openPrivilege = function (options, path) {
    if (typeof path == 'undefinde' || path == null || path == "") {
        options.code();
        return;
    }
    path = path.replace(projectName, "");
    if (path.indexOf("?") != "-1") {
        path = path.substring(0, path.indexOf("?"));
    }
    $.ajax({
        type: 'post',
        url: projectName + "/admin/sessionInfo/checkPrivilege.do",
        data: {"path": path},
        async: false,
        dataType: 'json',
        success: function (data) {
