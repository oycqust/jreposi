...
import com.utstar.integral.thread.LogHandleThread;
import com.utstar.integral.thread.StoreCacuResultThread;
...
public class StartUpListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    @Resource
    private LogHandleThread handleThread;
    @Resource
    private StoreCacuResultThread storeCacuResultThread;
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if(null == event.getApplicationContext().getParent()){
            ...
            if(cdrTime == null){
                ...
                log.info("set cdrTime success");
            }else{
                log.info("cdrTime is {}", cdrTime);
            }
//            redisCommonDAO.set(RedisKeyConstant.INTEGRAL_CDR_TIME, "201808082100");
            ...
            handleThread.start();
            LOGGER.info("LogHandleThreadc is starting....");
            storeCacuResultThread.start();
            LOGGER.info("StoreCacuResultThread is starting....");
    ...
    private void validate(){
        ...
        if(StringUtils.isEmpty(selfConfigReader.getDataDir())){
            ...
            if(!dataDir.exists()){
                isOk = false;
                errMsg = String.format("data.dir is not exists. data.dir is %s",
                        selfConfigReader.getDataDir());
            }
            if(!dataDir.isDirectory()){
                isOk = false;
                errMsg = String.format("data.dir is not be a directory. data.dir is %s",
                        selfConfigReader.getDataDir());
            }
        }
        if(!isOk){
