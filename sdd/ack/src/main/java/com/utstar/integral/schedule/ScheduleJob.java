...
public class ScheduleJob {
    ...
    private static final String MASTER_TRUE = "true";
...
//            LOGGER.info("{} release lock {} ",RedisKeyConstant.HOSTNAME,RedisKeyConstant.WORLD_MATCH_LOCK,releaseLock?"success":"failed");
    ...
    @Scheduled(cron = "${acti.mediacode.sync.cron}")
    public void syncMediacode2Redis() {
        if(StringUtils.isBlank(master) || !MASTER_TRUE.equals(master)){
            return;
        }
    ...
    @Scheduled(cron = "${acti.mediacode.view.cron}")
    public void syncActiTotalCode() {
        if(StringUtils.isBlank(master) || !MASTER_TRUE.equals(master)){
            return;
        }
        LOGGER.info("activity-schedule: sync total mediacode view redis start.");
        commonService.syncActiTotalCode();
        LOGGER.info("activity-schedule: sync total mediacode view to redis end.");
    }

    @Scheduled(cron = "${acti.check.code.cron}")
    public void checkValidActi() {
        if(StringUtils.isBlank(master) || !MASTER_TRUE.equals(master)){
            return;
        }
        LOGGER.info("activity-schedule: check valid code start.");
        commonService.checkValidActi();
        LOGGER.info("activity-schedule: check valid code end.");
    }
}
