...
public class RedisCommonDAOImpl<T> implements RedisCommonDAO<T> {
    ...
    @Override
    public void multiIncr(Map<String,Long> map,boolean isChange){
        ...
        Object o = stringRedisTemplate.execute(new RedisCallback<Object>() {
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                for(Map.Entry<String,Long> entry :map.entrySet()){
                    connection.incrBy(keySerializer.serialize(entry.getKey()), entry.getValue());
                }
    ...
    @Override
    public void multiIncr(Map<String,Long> map, long expireTime, boolean isChange){
        ...
        Object o = stringRedisTemplate.execute(new RedisCallback<Object>() {
            ...
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                for(Map.Entry<String,Long> entry :map.entrySet()){
                    connection.incrBy(keySerializer.serialize(entry.getKey()), entry.getValue());
                    connection.expire(keySerializer.serialize(entry.getKey()),expireTime);
                }
                return connection.closePipeline();
            }
        });
...
//                });
    ...
    public Set<String> sMembers(String key){
        try {
            ...
            log.error(String.format("dao-redis: fail to get data from set, the key is %s", key), e);
            return null;
        }
    }
    ...
    @Override
    public boolean multiAddSet(String key, Set<String> set) {
        try {
            stringRedisTemplate.opsForSet().add(key, set.toArray(new String[set.size()]));
            return true;
        }catch (Exception e){
            log.error("fail to add set.", e);
            return false;
        }
    ...
    @Override
    public boolean syncMediacode(String key, Set<String> set) {
        try {
            ...
            log.error(String.format("fail to sync mediacode", key), e);
            return false;
        }
        return true;
    }
    ...
    public List<String> multiGetForList(String key, int len) {
        List<Object> execute = stringRedisTemplate.execute((RedisCallback<List<Object>>) connection -> {
        ...
        execute.forEach((b -> {
            if (b != null && b instanceof byte[]) {
                ...
                result.add(valueSerializer.deserialize((byte[])b));
            }
        }));
        return result;
    }
    ...
    @Override
    public void multiLpush(String key, Collection<String> collection) {
